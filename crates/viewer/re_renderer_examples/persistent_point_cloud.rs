...
impl framework::Example for PointCloud {
    ...
    fn draw(
        ...
        // activate rave
        {
            ...
            let outline: Box<[_]> = (0..num_points)
                .map(|i| {
                    UVec2::new(
                        if i % a == 0 { 1 } else { 0 },
                        if i % b == 0 { 1 } else { 0 },
            ...
            let colors: Box<[_]> = (0..num_points)
                .map(|i| {
                    let v = ((i as f32 / num_points as f32) * 10.0 + seconds_since_startup * 6.0)
                        .sin()
                        * 0.5
                        + 0.5;
                    Rgba32Unmul([
            ...
            let points: Box<_> = FibonacciSphere::new(
                num_points as usize,
        ...
        view_builder.queue_draw(
            re_ctx,
            PersistentPointCloudDrawData::new(
                re_ctx,
